define([
    ...
    function init () {
        ...
        Appodeal.initialize(config.appodeal_app_key_android, Appodeal.INTERSTITIAL | Appodeal.SKIPPABLE_VIDEO | Appodeal.REWARDED_VIDEO);
        ...
        document.addEventListener('onSkippableVideoShown', function (data) { onShow('video', data); });
        document.addEventListener('onRewardedVideoShown', function (data) { onShow('rewardedVideo', data); });
        document.addEventListener('onInterstitialClosed', function (data) { onClose('interstitial', data); });
        document.addEventListener('onSkippableVideoClosed', function (data) { onClose('video', data); });
        document.addEventListener('onRewardedVideoClosed', function (data) { onClose('rewardedVideo', data); });
    }
    ...
    function show (params, onSuccess, onFail) {
        ...
        if (params && params.allowVideo) {
            allowedTypes.push(Appodeal.SKIPPABLE_VIDEO);
        }
    ...
    function checkTypePromise (type) {
        return new vow.Promise(function (resolve, reject) {
            ...
            Appodeal.isLoaded(type, function (result) {
                resolve(!!result);
                resolved = true;
            });
        })
